    ...
    * All vertices in the graph SHOULD BE UNIQUE! IT SHOULD NOT BE POSSIBLE
    ...
    def fix_edge(self, u: Vertex, v: Vertex) -> bool:
        ...
        if u not in self.vertices or v not in self.vertices:
            ...
            return False
        elif u not in v.edges or v not in u.edges:
            u.add_edge(v)
            v.add_edge(u)
            return True
        return False
    ...
    def block_edge(self, u: Vertex, v: Vertex) -> bool:
        ...
        if u not in self.vertices or v not in self.vertices:
            ...
            return False
        elif u in v.edges and v in u.edges:
            u.rm_edge(v)
            v.rm_edge(u)
            return True
        return False
    ...
    def find_path(
        ...
        path = [s]
        s.visited = True
        self.search_path(s, t, k, k-1, path)
        if self.path == [] or self.path == None:
            return None
        else:
            self.path = self.valid_path_check(self.path)
        return self.path  

    def search_path(self, u, destination, orig_food, curr_food, path):
        if u == None or u.edges == None:
            return
        u.visited = True
        for v in u.edges:
            if v.visited:
                ...
                v.parent = u
                path.append(v)
                self.search_path(v, destination, orig_food, orig_food - 1, path)
            elif not v.has_food:
                if curr_food > 0:
                    v.parent = u
                    path.append(v)
                    self.search_path(v, destination, orig_food, curr_food - 1, path)
                elif curr_food == 0:
                    continue

    def valid_path_check(self, path):
        path = self.reverse_path(path)
        for y in path:
            if path.index(y) == len(path) - 1:
                continue
            else:
                while path[path.index(y) + 1] != y.parent:
                    path.remove(path[path.index(y) + 1])
        path = self.reverse_path(path)
        return path
    ...
    def exists_path_with_extra_food(
        ...
        """
        self.path = None
        if s == None or t == None or k < 1 or x < 1:
            return False
        elif s not in self.vertices or t not in self.vertices:
            return False
        elif s == t:
            return True
        for u in self.vertices:
        ...
        path = [s]
        s.visited = True
        self.search_exists(s, t, k, k-1, x, path)
        if self.path == [] or self.path == None:
            return False
        else:
            return True

    def search_exists(self, u, destination, orig_food, curr_food, spare_food, path):
        if u == None or u.edges == None:
            return
        u.visited = True
        for v in u.edges:
            if v.visited:
                ...
                v.parent = u
                path.append(v)
                self.search_exists(v, destination, orig_food, orig_food - 1, spare_food, path)
            elif not v.has_food:
                if curr_food > 0:
                    v.parent = u
                    path.append(v)
                    self.search_exists(v, destination, orig_food, curr_food - 1, spare_food, path)
                elif curr_food == 0:
                    if spare_food > 0:
                        v.parent = u
                        path.append(v)
                        self.search_exists(v, destination, orig_food, orig_food - 1, spare_food - 1, path)
                    elif spare_food == 0:
                        continue